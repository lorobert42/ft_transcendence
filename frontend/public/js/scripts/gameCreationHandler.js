...
export async function gameCreationHandler(dataDict = {}) {
    ...
    document.getElementById('gameCreateSubmit').addEventListener('click', async (e) => {
        ...
        games.forEach(element => {
            if(foundGame) {return ;}
            if(((element.player1 == dataDict.user.id && element.player2 == dataDict.user.friends[playerSelect.value]) ||
            (element.player2 == dataDict.user.id && element.player1 == dataDict.user.friends[playerSelect.value])) &&
            (element.score1 == 0 && element.score2 == 0))
            {
                //redirect to ongoing party
                console.log("Redirecting to ongoing party with id :", element.id);
                console.log("With players: ", dataDict.user.id, " and ", dataDict.user.friends[playerSelect.value]);
        ...
        await fetch("/api/game/", {
            ...
            body: `{
                "player1": ${dataDict.user.id},
                "player2": ${dataDict.user.friends[playerSelect.value]},
                "score1": 0,
                "score2": 0
            }`,
          }).then((response) => {
