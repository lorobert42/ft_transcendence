...
class GameRoomConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        if GameRoomConsumer.game_tab.get(self.game_room_id) == None:
            GameRoomConsumer.game_tab['self.game_room_id'] = GameClass(self.game_room_id)

        """ Test to see if the game is created when is none existant. (can be deleted)"""
        try:
            print(GameRoomConsumer.game_tab['self.game_room_id'].count)
        except:
    ...
    async def receive(self, text_data):
        ...
        if message == "P1_UP":
            ...
            GameRoomConsumer.game_tab['self.game_room_id'].p1 = True
            print("P1 join")
        elif message == "P2":
            GameRoomConsumer.game_tab['self.game_room_id'].p2 = True
            print("P2 join")
        elif message == "start":
            if GameRoomConsumer.game_tab['self.game_room_id'].p1 == True and GameRoomConsumer.game_tab['self.game_room_id'].p2 == True:
                print("game started")
                GameRoomConsumer.game_tab['self.game_room_id'].active = True
                GameRoomConsumer.game_tab['self.game_room_id'].task = asyncio.create_task(self.loop(
                GameRoomConsumer.game_tab['self.game_room_id'].max_score
                ))

    def handle_paddle_move(self, key):
        """Function that will move the paddle in function of the key pressed"""
        left = GameRoomConsumer.game_tab['self.game_room_id'].paddle_l
        right = GameRoomConsumer.game_tab['self.game_room_id'].paddle_r
        if key is KEY_P1_UP and left.y - left.vel >= 0:
            GameRoomConsumer.game_tab['self.game_room_id'].paddle_l.move(up=True)
        elif key is KEY_P1_DOWN and left.y + left.vel + left.height <= HEIGHT:
            GameRoomConsumer.game_tab['self.game_room_id'].paddle_l.move(up=False)
        elif key is KEY_P2_UP and right.y - right.vel >= 0:
            GameRoomConsumer.game_tab['self.game_room_id'].paddle_r.move(up=True)
        elif key is KEY_P2_DOWN and right.y + right.vel + right.height <= HEIGHT:
            GameRoomConsumer.game_tab['self.game_room_id'].paddle_r.move(up=False)

    async def loop(self, max_score):
        """ Main loop that will run the Game. """
        print("in loop")
        while GameRoomConsumer.game_tab['self.game_room_id'].active is True:
            if GameRoomConsumer.game_tab['self.game_room_id'].score_p1 < GameRoomConsumer.game_tab['self.game_room_id'].max_score and \
            GameRoomConsumer.game_tab['self.game_room_id'].score_p2 < GameRoomConsumer.game_tab['self.game_room_id'].max_score:
                """ Game logic """
                GameRoomConsumer.game_tab['self.game_room_id'].ball.move()
                ball = GameRoomConsumer.game_tab['self.game_room_id'].ball
                left = GameRoomConsumer.game_tab['self.game_room_id'].paddle_l
                right = GameRoomConsumer.game_tab['self.game_room_id'].paddle_r
                if ball.y + ball.rad >= HEIGHT or ball.y - ball.rad <= 0: # Ball hit the ceiling
                    GameRoomConsumer.game_tab['self.game_room_id'].ball.y_vel *= -1
                # Cheking the direction of the ball
                    ...
                    if ball.y >= left.y and ball.y <= left.y + left.height and ball.x - ball.rad <= left.x + left.width:
                        ...
                        GameRoomConsumer.game_tab['self.game_room_id'].ball.x_vel *= -1
                        middle_y = left.y + left.height / 2
                        difference_in_y = middle_y - ball.y
                        reduction_factor = (left.height / 2) / ball.max_vel_y
                        y_vel = difference_in_y / reduction_factor
                        GameRoomConsumer.game_tab['self.game_room_id'].ball.y_vel = -1 * y_vel
                ...
                if ball.x <= 0: # right have scored so P2 won a point
                    GameRoomConsumer.game_tab['self.game_room_id'].score_p2 += 1
                    GameRoomConsumer.game_tab['self.game_room_id'].ball.reset()
                elif ball.x >= WIDTH: # left have scored so P1 won a point
                    GameRoomConsumer.game_tab['self.game_room_id'].score_p1 += 1
                    GameRoomConsumer.game_tab['self.game_room_id'].ball.reset()

                """ Getting data to send to the front """
                data = {
                    "P1": GameRoomConsumer.game_tab['self.game_room_id'].paddle_l.pos(),
                    "P2": GameRoomConsumer.game_tab['self.game_room_id'].paddle_r.pos(),
                    "Ball": GameRoomConsumer.game_tab['self.game_room_id'].ball.pos(),
                    "P1 Score": GameRoomConsumer.game_tab['self.game_room_id'].score_p1,
                    "P2 Score": GameRoomConsumer.game_tab['self.game_room_id'].score_p2,
                }
                await self.channel_layer.group_send(
                    self.game_room_group,
                    {
                ...
                GameRoomConsumer.game_tab['self.game_room_id'].active = False
